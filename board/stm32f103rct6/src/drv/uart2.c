...
static uint8_t *u2_r_buff = NULL;
static uint8_t *u2_t_buff = NULL;
...
void _uart2_dma_configuration()
{
	...
	DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)0;						// 发送内存地址
...
void USART2_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART2, USART_IT_IDLE) != RESET) //空闲中断
	{
		...
		temp = temp;
		int len = BUFF_SIZE_UART - DMA1_Channel6->CNDTR;
		for (int i = 0; i < len; i++)
		{
