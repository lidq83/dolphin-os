...
static uint8_t statck_t[STACK_SIZE];
static uint8_t statck_t2[STACK_SIZE];
...
//    pcb_create(24, &task_0, NULL, &stack_task[0][STACK_SIZE]);
	pcb_create(0, task_1, NULL, &statck_t[STACK_SIZE]);
	pcb_create(1, task_2, NULL, &statck_t2[STACK_SIZE]);
}
